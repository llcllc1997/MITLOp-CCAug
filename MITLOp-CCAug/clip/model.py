...
class ResidualAttentionBlock_MITLOp(nn.Module):
    def __init__(self,
                 d_model: int,
                 n_head: int,
                 attn_mask: torch.Tensor = None,
        ...
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
        self.vp_length = design_details["vp_length"]
        if i == 0:
        ...
        if self.first_layer:
            if len(compound_prompts_deeper) > 0:
                ...
                if not (counter > len(compound_prompts_deeper) - 1):
                    ...
                    # print(self.vp_length)
                    prefix = x[0:x.shape[0] - self.vp_length, :, :]
...
class Transformer_T(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        hidden_features = []
        for i in range(len(self.resblocks)-1):
            x = self.resblocks[i](x)
            hidden_features.append(x.unsqueeze(dim=0))
        hidden_features = torch.cat(hidden_features, dim=0)
        return x, hidden_features

class Transformer_MITLOp(nn.Module):
    def __init__(self,
                 width: int,
                 layers: int,
                 heads: int,
        ...
        self.layers = layers
        self.resblocks = nn.Sequential(*[
            ResidualAttentionBlock_MITLOp(width, heads, attn_mask, design_details)
            for _ in range(layers)
        ])
...
class VisionTransformer_MITLOp(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int,
        ...
        self.transformer = Transformer_MITLOp(width, layers, heads, design_details=design_details)

        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))

    def forward(self, x: torch.Tensor, vp, dense_vp):
        ...
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)
        vp = vp.expand(x.shape[0], -1, -1)
        x = torch.cat([x, vp], dim=1)
        x = x.permute(1, 0, 2)  # NLD -> LND
        outputs = self.transformer([x, dense_vp, 0])
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class CLIP(nn.Module):
    def __init__(
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer_MITLOp(input_resolution=image_resolution,
        ...
        self.transformer = Transformer_T(width=transformer_width,
