...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        x = x[torch.arange(x.shape[0]), 10] @ self.text_projection

        return x

#class CLSLearner(nn.Module):
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # print(ctx_vectors.unsqueeze(0).shape)
        ...
        # print(cls_embedding[0,:,0])
        cls_vectors = torch.zeros(n_cls, 4, ctx_dim, dtype=dtype)
        cls_vectors[:, :, :] = cls_embedding[:, 1:5, :]
        # print(cls_vectors)
        ...
        # ending_token = clip.tokenize(prompt_prefix)  # without "."
        # suffix_token = ending_token[:, 1 + n_ctx: -4]
    ...
    def forward(self):
        ...
        content = torch.cat((ctx,clsv), dim=1)
        # print(content.shape)
        # suffix = torch.zeros(1, 76-10-n_ctx)
        # suffix[]
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        prompts, clsv = self.prompt_learner()
        ...
        # cls_div = clsv.reshape(clsv.shape[0], -1)
        # cls_div = cls_div / cls_div.norm(dim=-1, keepdim=True)
        # print(cls_div.shape)
        # target = torch.eye(cls_div.shape[0]).cuda() 
        # source = torch.matmul(cls_div, cls_div.transpose(0, 1))
        # for i in range(100):
        #     print(source[i,i])
        # print("*********************")
        # div_loss = torch.norm(target - source, dim=(0, 1))
