...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        x = x[torch.arange(x.shape[0]), 13] @ self.text_projection
        # print("lllllllllll")
        # print(x.shape)
        # print("lllllllllll")
        # print(tokenized_prompts.argmax(dim=-1).shape)
        return x


class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, classtokens, clip_model):
        super().__init__()
        ...
        # kdot = "."
        # print(clip.tokenize(kdot))
        # print(tokenized_prompts[0])
        # print(tokenized_prompts[-1])       
        ...
        ending_token = clip.tokenize(prompt_prefix)
        suffix_token = ending_token[:, 1 + n_ctx: -8]
        with torch.no_grad():
            suffix_embedding = clip_model.token_embedding(suffix_token).type(
                dtype)
        suffix_embedding = suffix_embedding.expand(n_cls, -1, -1)
        self.register_buffer("token_suffix", suffix_embedding)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, classtokens, clip_model):
        super().__init__()
        self.prompt_learner = PromptLearner(cfg, classnames, classtokens, clip_model)
...
class CoOp(TrainerX):
    ...
    def build_model(self):
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            print("Loading optimized class name tokens")
            checkpoint = load_checkpoint(cfg.MODEL.INIT_WEIGHTS)
            if "state_dict" in checkpoint:
                state_dict = checkpoint["state_dict"]
            classtokens = state_dict['cls']
            print(classtokens.shape)
        ...
        self.model = CustomCLIP(cfg, classnames, classtokens, clip_model)

        print("Turning off gradients in both the image and the text encoder")
        for name, param in self.model.named_parameters():
            if "prompt_learner" not in name:
                param.requires_grad_(False)
        # print(self.model.prompt_learner)
        # if cfg.MODEL.INIT_WEIGHTS:
